...
class LDNet(nn.Module):
    ...
    def forward(self, input):
        out = self.conv(input)
        out = out.view(input.size(0), 100)
        out = self.linear(out)
        return out.view(input.size(0), 5, 2)

def define_LD(gpu_ids=[]):
    """ define Landmark detector network on fixed size """
    net = LDNet()
...
class GANLossLD(nn.Module):
    ...
    def __init__(self):
        super(GANLossLD, self).__init__()
        self.loss_function = nn.MSELoss(True)
        print('Hello from GANLossLD')
    
    def __call__(self, predictions, targets):
        """ expects the landmarks in a 5x2 tensor """
        
        p = predictions#.view(5,2)
        t = targets#.view(5,2)
